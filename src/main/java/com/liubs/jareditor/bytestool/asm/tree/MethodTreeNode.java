...
public class MethodTreeNode extends BaseTreeNode{
    ...
    public MyInstructionInfo getInstructionInfo(){
        ...
        List<MyInstructionInfo.LineNumber> markLines = new ArrayList<>();
        List<FrameNode> frameNodes = new ArrayList<>();

        /** 先遍历LabelNode构建索引编号*/
        Map<LabelNode,Integer> labelIndexMap = this.buildLabelIndex();
        ...
        while (instructions.hasNext()) {
            ...
            if (currentInsn instanceof InsnNode) {
                ...
                MyInstructionInfo.LineNumber lineNumber = new MyInstructionInfo.LineNumber();
                ...
                for(int i = markLines.size()-1 ; i>=0 ;i--){
                    MyInstructionInfo.LineNumber lineNumber = markLines.get(i);
